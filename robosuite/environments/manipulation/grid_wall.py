...
from robosuite.models.objects import BlockObject, GridWallObject, GridObject
...
class GridWall(SingleArmEnv):
    """
    Environment for stimulus collection. There is no success or failure.
    Abstract states are defined as:
    ...
    def __init__(
        ...
        obj_intial_abs_state=None, # initial position of object specified by abstract state (int from 1 to 9). if unspecified, randomly chosen
        grid_height=0.10, # height of grid walls
        grid_rgba=(0.5,0.5,0.5,1.0),
    ):
        ...
        self.workspace_y = (-0.5 * self.table_full_size[1] + self.table_offset[1], 0.5 * self.table_full_size[0] + self.table_offset[1])
        self.workspace_z = (self.table_offset[2], self.table_offset[2] + 1.0)

        # initial eef position
        self.initial_eef_pos = np.array(initial_eef_pos) + np.array(self.table_offset) + np.array([0, 0, 0.05])

        # object
        self.obj_half_size = obj_half_size
        self.obj_rgba = obj_rgba
        self.obj_initial_abs_state = obj_intial_abs_state

        # grid
        self.grid_height = grid_height
        self.grid_rgba = grid_rgba
        self.grid_half_size = np.array([self.table_full_size[0]/2, self.table_full_size[1]/2, self.grid_height/2])
        ...
        # abstract states (grid cell centers)
        center_x = [-self.grid_half_size[0]*2/3, 0, self.grid_half_size[0]*2/3]
        center_y = [-self.grid_half_size[1]*2/3, 0, self.grid_half_size[1]*2/3]
        self.abstract_states = {
            1 : np.array([center_x[0], center_y[0]]),
            2 : np.array([center_x[0], center_y[1]]),
            3 : np.array([center_x[0], center_y[2]]),
            4 : np.array([center_x[1], center_y[0]]),
            5 : np.array([center_x[1], center_y[1]]),
            6 : np.array([center_x[1], center_y[2]]),
            7 : np.array([center_x[2], center_y[0]]),
            8 : np.array([center_x[2], center_y[1]]),
            9 : np.array([center_x[2], center_y[2]]),
        }
    ...
    def _load_model(self):
        ...
        self.object = BlockObject(
            name="block",
            body_half_size=self.obj_half_size,
            rgba=self.obj_rgba,
            density=500
        )

        # initialize grid object
        self.grid = GridWallObject(
            name="grid",
            body_half_size=self.grid_half_size,
            # body_half_size=(0.3, 0.3, 0.05),
            wall_thickness=0.01,
            rgba=self.grid_rgba,
        )
        ...
        self.model = ManipulationTask(
            ...
            mujoco_objects=[self.object, self.grid],
        )
    ...
    def _reset_internal(self):
        ...
        # place grid in center of the table
        self.sim.data.set_joint_qpos(self.grid.joints[0], np.array([0, 0, self.table_offset[2] + 0.5*self.grid_height, 0, 0, 0, 1]))

        # place object in one of the grid cells
        if self.obj_initial_abs_state is None:
            self.obj_initial_abs_state = np.random.randint(1, 10)

        obj_pos = np.array([
            self.abstract_states[self.obj_initial_abs_state][0],
            self.abstract_states[self.obj_initial_abs_state][1],
            self.table_offset[2] + 0.5*self.obj_half_size[2],
            0, 0, 0, 1
        ])
        self.sim.data.set_joint_qpos(self.object.joints[0], obj_pos)
    ...
    def _check_success(self):
        # there is no success condition for this environment
        return False
