...
class Object_and_Table(SingleArmEnv):
    """
    ...
    def _reset_internal(self):
        ...
        # # place object in good position to grasp
        # self.sim.data.set_joint_qpos(
        #     self.object.joints[0], np.array([-0.37, 0, self.table_offset[2]+0.15, 0, 0, 0, 1])
        # )
        # take a few steps to grip the block
        for j in range(100):
            # place object in gripper
            self.sim.data.set_joint_qpos(
                self.object.joints[0], np.concatenate([self._eef_xpos, np.array([0, 0, 0, 1])])
            )
